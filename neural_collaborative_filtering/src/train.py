...
def main():
    ...
    try:
        ...
        id_mappings = {
            'user_id_to_idx': {str(k): v for k, v in user_encoder.classes_.tolist() if v < len(user_encoder.classes_)},
            'movie_id_to_idx': {str(k): v for k, v in item_encoder.classes_.tolist() if v < len(item_encoder.classes_)},
            'idx_to_user_id': {v: str(k) for k, v in user_encoder.classes_.tolist() if v < len(user_encoder.classes_)},
            'idx_to_movie_id': {v: str(k) for k, v in item_encoder.classes_.tolist() if v < len(item_encoder.classes_)}
        }
